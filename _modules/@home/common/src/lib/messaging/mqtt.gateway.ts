import { IClientSubscribeOptions, MqttClient, connect } from 'mqtt';
  ...
  public setup(config: MqttConfiguration, router: IRoutable): void {
    ...
    if (this._client) {
      ...
      this._client.on('message', (topic, message) => {
        const routeMessage = JSON.parse(message.toString());
        const payload = new MqttMessage(topic, routeMessage);
